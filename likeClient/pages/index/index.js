...
Page({
    ...
    _hadlerTap:function(){
        wx.navigateTo({
          url: '/pages/01-zujian/01-zujian?id=1',
        })
    },
    //事件绑定： 点击按钮 插入数据到数组
  ...
  data: { 
      ...
      ceshiNumber:200550,
      isNb:true, //条件渲染
      score:100, //wx:if  wx:else 

      ceList:[
        {name:'视频案例',path:'/pages/06-video/06-video'},
        {name:'07案例',path:'/pages/07-dianshang/07-dianshang'},
        {name:'云开发交友点赞',path:'/pages/08-jiaoyou/08-jiaoyou'},
        {name:'地图-附近好友',path:'/pages/09-near/09-near'}
      ]
  },
  /**
   * 生命周期函数--监听页面加载
  ...
  /**
   * 生命周期函数--监听页面显示，当小程序启动，或从后台进入前台显示
   */
  onShow: function () {
      console.log("C2:首页-监听页面显示-onShow-2-切换到前台");
  },
   ...
   */
  onHide: function () {
      console.log("D2:首页-监听页面隐藏-切换到后台-可以暂停播放等操作");
  },
