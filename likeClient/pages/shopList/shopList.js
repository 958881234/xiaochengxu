Page({
  ...
  loadMore: function () {
      ...
      wx.request({
          ...
          data: {
              _limit: this.data.pageSize, //多少条
              _page: ++this.data.pageIndex //多少页，每次请求++
          },
          success: (res) => {
              console.log(res);
              // bug：请求过来的数据把本来的替换掉了
              // 1.6. 解决bug思路，先获取本来的数据，再通过concat方法把新数据拼接起来。
              var newList = this.data.shopList.concat(res.data);
              // 2.3 获取数据的总数 他存在header中了
              var count = parseInt(res.header['x-total-count']);
              console.log(count)
              // 2.4 用于判断比较是否还有更多数据
              var flag = this.data.pageIndex * this.data.pageSize < count;
              this.setData({ //this.setData 微信带功能，1更新数据，2更新视图
                  shopList: newList,
                  hasMore: flag,
              });
          },
